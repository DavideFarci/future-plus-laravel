                    ...
                    <tbody>
                        @foreach ($custumers as $custumer)
                            <tr class="odd:bg-slate-300 dark:odd:bg-slate-700 hover:opacity-60">
                                ...
                                >
                                    <a class="block h-full p-2" href="{{ route('admin.email.show', $custumer->id) }}">
                                ...
                                >
                                    <a class="block h-full p-2" href="{{ route('admin.custumers.show', $custumer->id) }}">
                                        {{ date('d/m/Y H:i', strtotime($custumer->created_at)) }}
                                    </a>
                                </td>
                                <td class="p-2 text-sm text-right ">
                                    <form action="{{ route('admin.custumers.destroy', ['custumer' => $custumer]) }}" method="POST">
                                        @csrf
                                        @method('delete')
